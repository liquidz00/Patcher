...
The repo root also contains an `api/` directory (sibling to `src/`) — the Patcher API service, a workspace member declaring its own `patcher-api` package. Imports `patcherctl` via `[tool.uv.sources] patcherctl = { workspace = true }` so it can call `patcher.core.installomator.parse_fragment` and `resolve` (the pyinstallomator shell-expression evaluator) without code duplication. Not packaged into the Patcher Python distribution; deployed separately. The API surface is FastAPI-based, backed by SQLAlchemy 2.0 async + SQLite, with bearer-token auth (SHA-256 hashed). See `api/patcher_api/` for routes/models/schemas/ingest/stitch logic.
