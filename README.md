...
## Patcher API (private beta)

A separate workspace member at [`api/`](api/) powers a community catalog of macOS app patching metadata available at <https://api.patcherctl.dev>. The catalog stitches data from Installomator, Homebrew Cask, the Mac App Store, AutoPkg, and the Jamf App Installers index into a single queryable surface.

The API is currently in private beta and requires a bearer token for access. To request one, ping the [#patcher channel](https://macadmins.slack.com/archives/C07EH1R7LB0) in MacAdmins Slack. Endpoint reference and `curl` + `httpx` examples are at [docs.patcherctl.dev/api/endpoints](https://docs.patcherctl.dev/api/endpoints.html).
