...
The CLI (`patcherctl`) is the primary interface for most users. The underlying logic is also importable. See the [library quickstart](https://patcher.readthedocs.io/getting-started/setup/library.html) for using Patcher programmatically from your own scripts or services.
...
> [!NOTE]
> Patcher is both a CLI tool and a Python library. Most users will reach for `patcherctl`, but the underlying `patcher` package is importable for custom integrations. See the [library quickstart](https://patcher.readthedocs.io/getting-started/setup/library.html) for the importable surface.
