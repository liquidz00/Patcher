...
The CLI (`patcherctl`) is the primary interface for most users. The underlying logic is also importable. See the [library quickstart](https://docs.patcherctl.dev/getting-started/setup.html#library-quickstart) for using Patcher programmatically from your own scripts or services.
...
> [!NOTE]
> Patcher is both a CLI tool and a Python library. Most users will reach for `patcherctl`, but the underlying `patcher` package is importable for custom integrations. See the [library quickstart](https://docs.patcherctl.dev/getting-started/setup.html#library-quickstart) for the importable surface.
...
A separate workspace member at [`api/`](api/) powers a community catalog of macOS app patching metadata at <https://api.patcherctl.dev>. The catalog stitches data from Installomator, Homebrew Cask, the Mac App Store, AutoPkg, and the Jamf App Installers index into a single queryable surface.

Read endpoints are public — no authentication required. Endpoint reference and `curl` + `httpx` examples are at [docs.patcherctl.dev/api/endpoints](https://docs.patcherctl.dev/api/endpoints.html).
