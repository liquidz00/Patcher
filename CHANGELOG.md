...
- **Internal (API): one per-record savepoint primitive for ingest and stitch.** The Installomator ingest (two loops) and the catalog stitch each had their own per-record error handling — the ingest committed every row and rolled back on a broad `except Exception`, while stitch used a savepoint with the same broad catch. Both now route through a shared `patcher_api.db.run_in_savepoint` / `execute_in_savepoint` (one SAVEPOINT per record, one commit at the end of the batch). The catch is narrowed to `SQLAlchemyError` and logs with a traceback, so a genuine bug now surfaces loudly instead of being silently tallied as a "failed record"; only real database errors are isolated and counted. The Homebrew/AutoPkg ingest keep their validate-first model (they reject bad records before the write).
- **Internal (API): the MCP tools and REST routes share one service layer.** `get_app`, `get_app_sources`, `generate_label`, and `list_drift` each had a near-identical copy of its fetch/project logic in both `routes/apps.py` and `mcp/tools.py` (and a third copy of the app lookup in `mcp/resources.py`), which is how a fix could land in one and not the other. The logic now lives once in a new `patcher_api.services` module that raises domain exceptions (`AppNotFound`, `NoSourceDetail`); each surface translates them to its own error shape (REST to `HTTPException`, MCP to a tool error). No change to either surface's responses.
- **`import patcher` no longer eagerly loads pandas, keyring, and fpdf.** The package's public symbols are now imported lazily (only when first accessed), so a leaf import like `import patcher.catalog` — the wire schemas the API server and library-only consumers use — pulls in none of the CLI/report heavyweights it doesn't need. The install command and the public API are unchanged: `pip install patcherctl` still ships the full library and CLI, and every `from patcher import ...` works exactly as before (just resolved on demand). The keyring backend is now configured at the credential boundary (`ConfigManager`) rather than at package import, which also fixes credential code that imported `ConfigManager` directly without going through `import patcher` first.
