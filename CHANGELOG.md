...
## [Unreleased]
### Tests
- **End-to-end Setup `start()` test added** at `tests/test_setup.py::test_start_persists_credentials_via_jamf_credentials`. Exercises the real method body with only I/O collaborators mocked. Asserts the credential-persistence step constructs `JamfCredentials` (not `JamfClient`) and passes an `AccessToken` to `config.create_client`. The earlier mock-`start()`-entirely pattern hid the v2.5 NameError bug; this catches the same regression class going forward.
- **Subprocess fallback failure modes covered.** `api/tests/test_installomator_resolver.py::TestSubprocessFallback` gains three tests: non-zero exit returns `Unresolvable` with truncated stderr (200-char cap); 30s timeout via mocked `subprocess.run`; OSError starting bash (e.g. missing `/bin/bash` on a stripped container). All three previously had zero coverage despite being the same code path flagged as the top security finding.
- **JAI HTML parser malformed-input coverage.** Two new tests in `api/tests/test_ingest_jamf_app_installers.py`: cell count not divisible by 3 emits the divisibility warning and still returns the well-formed rows; an out-of-order cell sequence (entry__1 re-entering before entry__3 closes the previous row) logs the incomplete row and skips it. The anchor on `headers="reference-7022__entry__N"` is fragile; these tests fail loudly if Jamf restructures the page.
- **Admin upload edge cases.** Two new tests in `api/tests/test_admin.py`: expired deploy token returns 401 (covers the new Phase 3 `expires_at` column), and a streaming upload that omits Content-Length still hits the size cap mid-stream with `.tmp` cleanup verified. Caught a real bug in the Phase 3 `expires_at` comparison: SQLite returns offset-naive datetimes for `DateTime(timezone=True)` columns, so the naive-vs-aware comparison in `auth.py` would have crashed in production. Fixed by normalizing the stored timestamp to UTC before comparison.
- **PDF document-level rendering tests.** Three new tests in `tests/test_pdf.py`: full document written to disk validates as a real PDF (`%PDF-` magic + non-trivial byte count); multi-page rendering (60 rows → multiple pages with table-header rerun on page 2+); custom-font fallback to Helvetica when configured TTF paths don't exist on disk. Previous coverage stopped at helper functions and side-effect-only header calls.

### Security
- **`JamfCredentials.client_secret` and `AccessToken.token` are now `pydantic.SecretStr`.** Accidental serialization through `repr`, `model_dump`, `str()`, or unhandled exception tracebacks now renders `"**********"` instead of the actual secret. Callers that need the plaintext (the OAuth token request body, the `Authorization` header, the keychain write) call `.get_secret_value()` explicitly. Tests that asserted `token.token == "value"` now use `token.token.get_secret_value() == "value"`.
- **Pydantic `ValidationError` strings scrubbed before logging.** Both `TokenManager.attach_client` (`src/patcher/clients/token_manager.py:96`) and `TokenManager.ensure_valid_token` (`:207`) caught `ValidationError` and did `error_msg=str(e)`. Pydantic's default `__str__` includes the offending field VALUES, which here could include `client_id`/`client_secret`/`server`/`token` fragments. Both sites now extract only the field NAMES from `e.errors()` and surface those instead.
- **Deploy tokens now carry an `expires_at` column.** `api/patcher_api/models/deploy_token.py` adds the column (nullable for backward compatibility) and `api/scripts/grant_deploy_token.py` defaults new tokens to a 90-day lifetime via the new `DEFAULT_LIFETIME` constant. `get_current_deploy_token` returns 401 on tokens past their expiry. Tokens minted before this column existed have `expires_at = NULL` and continue to work (treated as "never expires") so the change is non-breaking; rotate them when convenient. **Production migration required:** on the Linode SQLite DB, run `ALTER TABLE deploy_tokens ADD COLUMN expires_at TIMESTAMP NULL;` once before redeploying. New tokens minted afterward have expiry set automatically.
- **`slowapi` rate limit on `/admin/*` routes.** Adds `slowapi>=0.1.9` to `api/pyproject.toml`. The catalog-upload endpoint is now capped at 12 requests per hour per IP via a `@limiter.limit("12/hour")` decorator, generous enough for the daily refresh-catalog CI run plus occasional manual retries, tight enough that a leaked deploy token + brute-force loop gets shut down before doing real damage. `/apps*` reads and `/health` stay unlimited; production protection there is Cloudflare's edge rate limiting.
- **Jamf server URL forced to `https://`.** `JamfCredentials.valid_url` (`src/patcher/core/models/jamf.py`) previously honored whatever scheme the user typed, so `http://my.jamfcloud.com` resulted in bearer tokens shipped over plain HTTP. The scheme is now hardcoded to `https`; HTTP inputs are silently upgraded.
- **`If-None-Match` header parsing now handles RFC 7232 multi-value lists and the wildcard.** Previously `api/patcher_api/main.py:117` did byte-exact equality; clients sending `If-None-Match: W/"a", W/"b"` or `*` were never matched and always got a full body back. The middleware now splits on `,`, strips whitespace, and treats `*` as a match.
...
### Fixed
- **CLI setup flow's credential-persistence step now actually runs.** `src/patcher/cli/setup.py:484` was instantiating `JamfClient(client_id=..., client_secret=..., server=...)`, but `JamfClient` (the HTTP client) wasn't imported in `setup.py` *and* its constructor doesn't accept those kwargs. Both errors trace to the v2.5 rename of the credentials Pydantic model from `JamfClient` to `JamfCredentials`. Anyone running `patcherctl --setup` end-to-end would hit a `NameError` at the final step. Fix: import `JamfCredentials` and pass that. Existing `tests/test_setup.py::test_run_setup_*` mock `Setup.start()` entirely so the broken block never ran in CI; an end-to-end smoke test would have caught this and is queued as a test-coverage gap.

### Removed
- **`FilterCriteria` and `TrendCriteria` enums, the `Analyzer` dispatch wrapper, and `BaseEnum.from_cli`** at `src/patcher/core/analyze.py`. Replaced by `TitleFilter` and `TrendAnalysis` classes whose methods correspond one-to-one with the former enum values. Library callers construct + invoke directly: `TitleFilter(titles).most_installed(top_n=10)`, `TrendAnalysis(datasets).patch_adoption()`. Per-method signatures replace the union `(threshold, top_n)` of the old `Analyzer.filter_titles`, so `below_threshold(threshold=70)` and `zero_completion()` only accept what they actually use. CLI strings (`patcherctl analyze --criteria most-installed`) and `PatcherClient.analyze("most-installed", ...)` still work via the new `TitleFilter.apply` / `TrendAnalysis.apply` dispatch helpers (use `inspect.signature` to thread only the kwargs each method accepts). `Analyzer.format_table` moved to a module-level helper in `patcher.cli` since it's pure CLI formatting with no domain dependencies. Top-level imports change: `from patcher import TitleFilter, TrendAnalysis` (was `FilterCriteria`, `TrendCriteria`). Library callers using `FilterCriteria.MOST_INSTALLED` or `TrendCriteria.PATCH_ADOPTION` will see `ImportError`; either switch to the new classes or pass the kebab-case string form to `PatcherClient.analyze`. `{versionchanged} 3.0` directives are present on the new classes and on the affected `PatcherClient.analyze*` methods so the rename surfaces in the rendered docs.
- **`InstallomatorClient.match` and its helpers** (`_match_directly`, `_match_fuzzy`, `_second_pass`, `_save_unmatched_apps`, `_normalize`, `_parse`, plus the local `IGNORED_TITLES` list and `self.review_file` / `self.threshold` instance attributes) at `src/patcher/clients/installomator.py`. The canonical matcher has been `patcher.core.matching.match_titles` since the API-backed matching pipeline shipped; `InstallomatorClient.match` had no live callers and duplicated the algorithm. The remaining surface (`list_available_labels`, `get_label`, `get_labels`, `parse_fragment`) is the documented label-discovery API. Library callers using `InstallomatorClient.match()` directly will see `AttributeError`; switch to constructing a `PatcherClient` and calling `fetch_patches`, which runs the same matching internally.
- **13 dead-code tests** in `tests/test_installomator.py` covering the removed methods (`test_match_*`, `test_normalize_*`, `test_match_directly_*`, `test_match_fuzzy_*`, `test_match_second_pass_*`). The remaining 15 tests cover the live surface.

### Changed
- **`PatcherAPIClient` and `HTTPClient` share a single `_request` helper for network-error translation.** Previously each had its own `with self.semaphore` + `try/except httpx.RequestError` block that re-raised as `APIResponseError`. The helper lives on `HTTPClient` (`src/patcher/clients/__init__.py`); subclasses with custom response parsing (e.g. `PatcherAPIClient._parse` handling FastAPI's `detail` body shape) call `self._request(method, url, **kwargs)` and parse the returned `httpx.Response` themselves. Public exception contracts are unchanged.
- **`parse_fragment` copied to `api/patcher_api/ingest/_label_parser.py`.** This was the only `from patcher.*` import in the entire api workspace; the parser is 50 lines of pure regex with no transitive deps on the library, so the duplication cost is near zero and the workspaces now have independent release cadence. Both copies are documented as deliberate duplicates with a note to update both intentionally if parsing behavior changes.
- **`InstallomatorWarning` now actually fires.** When `core.matching.match_titles` finishes with unmatched titles, it emits an `InstallomatorWarning` via `warnings.warn()` alongside the existing log line and review-file write. The CLI's existing `warnings.simplefilter("always", InstallomatorWarning)` surfaces it to end users; library callers can catch it programmatically (`warnings.catch_warnings`) or suppress with `filterwarnings("ignore", category=InstallomatorWarning)`. Previously the exception class was defined and exported but never raised.

### Fixed (continued)
- **`PatcherClient.export` now reads the configured `header_text` from `ui_config`** (`src/patcher/core/patcher_client.py:392`). Previously the lookup used the legacy uppercase `"HEADER_TEXT"` key while the modern UI config writes lowercase `"header_text"` (per `UIConfigKeys`), so the configured header was silently ignored and reports always rendered with the literal `"Patch Report"` fallback. The migration path in `plist_manager.py` still reads uppercase from v1 plists by design; the bug was in the runtime lookup, not the migration.
- **`header_color` default consolidated.** Previously the value `"#6432bdff"` was hardcoded in three places (`UIDefaults.header_color`, `PatcherClient.export`, `DataManager.export`), so changing one without the others would shift behavior depending on call path. The two `export` signatures now default to `None` and defer to `UIDefaults().header_color` via the existing `DataManager._export_html` fallback. Callers passing an explicit `header_color=` are unaffected.
...
### Changed
- **`patcher` now auto-configures a null keyring backend on non-macOS platforms.** Previously, importing `patcherctl` on Linux without a Secret Service / D-Bus session keyring would crash with a `keyring` `RuntimeError` before any user code ran, forcing every CI workflow and Docker container to set `KEYRING_BACKEND=keyring.backends.null.Keyring` by hand. The package now detects the platform at import time and installs `keyring.backends.null.Keyring` whenever `platform.system() != "Darwin"`, so Linux runners, Linux servers, Docker containers, and Windows hosts all just work. An explicit `KEYRING_BACKEND` env var still takes precedence for callers who want a specific backend. Behavior on macOS is unchanged.
- **`patcher.core.models.jamf.JamfClient` (Pydantic credentials model) renamed to `JamfCredentials`.** The name `JamfClient` previously referred both to the credentials Pydantic model under `patcher.core.models.jamf` and to the HTTP client at `patcher.clients.jamf.JamfClient`. The collision was resolved in favor of the HTTP client; the credentials model takes the more descriptive name. Library callers importing `from patcher.core.models.jamf import JamfClient` will see `ImportError`; switch the import to `JamfCredentials`. The top-level `from patcher import JamfClient` was already the HTTP client and is unchanged.
