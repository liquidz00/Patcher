...
## [Unreleased]
### Added
- **Jamf App Installers ingest moved from HTML scraping to the titles API.** The `learn.jamf.com` HTML scrape is replaced by Jamf Pro's App Installers *titles* API (`fetch_jai_catalog`: OAuth client-credentials, one token for the whole sweep — the title list plus bounded-concurrency per-title detail, each title falling back to its lean record if its detail call fails). The catalog-global title endpoints need no specific tenant, so ingest defaults to Jamf's public dummy instance (override via `PATCHER_API_JAI_BASE_URL` / `_CLIENT_ID` / `_CLIENT_SECRET`). `jamf_app_installers` gains `bundle_id`, `version`, `jamf_id`, `download_url`, `architecture`; `/apps/{slug}/sources` exposes them. Stitch matches JAI by **`bundle_id` first** (a precision overlay over the name matcher, which stays the workhorse since only ~16% of labels and no casks carry one) and **backfills** JAI's `bundle_id` onto name-matched apps that lack one — lifting catalog apps with a known `bundle_id` from ~179 to ~527 in practice. **Migration**: `ALTER TABLE jamf_app_installers ADD COLUMN bundle_id TEXT; ADD COLUMN version TEXT; ADD COLUMN jamf_id TEXT; ADD COLUMN download_url TEXT; ADD COLUMN architecture TEXT;` then `CREATE INDEX ix_jamf_app_installers_bundle_id ON jamf_app_installers (bundle_id);`.
- **macOS label resolution pipeline** for the values the Linux resolver can't compute faithfully. A label is a zsh program (`getJSONValue` via osascript, `$(arch)`, vendor curls), so a scheduled GitHub macOS runner (`.github/workflows/resolve-labels.yml`) runs `.github/scripts/resolveLabel.sh` against a fresh upstream Installomator checkout and POSTs the NDJSON to a new admin endpoint. Resolution and serving stay split: the public read path executes nothing, the macOS work runs offline ahead of request time. The runner only resolves the *worklist* the API hands it (labels Linux couldn't resolve, plus macОS-owned ones kept fresh), not every label — Linux refreshes what it can on the box for free. `resolveLabel.sh` lives in Patcher (not Installomator): takes `INSTALLOMATOR_DIR`, an `--all` flag, a concurrency pool (`--jobs`), and a per-call curl timeout (`--timeout`) so a dead vendor host can't stall the run.
- **`GET /admin/labels/unresolved` + `POST /admin/labels/resolved`** — the macOS resolver's read/write handshake. Reads of catalog data stay public; these two routes are gated by a shared `PATCHER_API_ADMIN_TOKEN` (fail-closed: unset = refuse every request). The GET returns the worklist (dynamic fields Linux left NULL, plus `macos`-owned rows), minus a small `USER_CONTEXT_LABELS` set (the `*intl` / language-pack variants whose URL resolves from the logged-in user's language — unresolvable on any headless host, so never worth sending). The POST validates each value with the same `looks_like_clean_http_url` / `looks_like_clean_version` checks the API serves through, **updates only** the `download_url` / `app_new_version` of labels the Linux ingest already created (never creating rows), skips `ok:false` records so a failed scrape can't clobber good data, then re-stitches and refreshes the `/apps*` ETag (with a WAL checkpoint so the file hash reflects the write). Adds nullable `resolution_source` + `resolved_at` columns to `installomator_labels`; the Linux refresh defers to fresh `macos`-stamped rows and reclaims them only after a 7-day staleness window. **Migration**: `ALTER TABLE installomator_labels ADD COLUMN resolution_source TEXT; ADD COLUMN resolved_at DATETIME;`.
...
- **SHA-gated Installomator ingest.** Discovery now reads GitHub's git/trees API (one call returns every fragment plus its blob SHA) instead of fetching `Labels.txt` + every `.sh`. Labels whose blob SHA matches the stored value are skipped entirely (no fetch, no parse, no resolver call). Steady-state ingest typically processes <20 labels per run instead of 700+. Labels removed upstream are auto-deleted with a log line listing what was removed. `--force` on `scripts/ingest.py` (or `PATCHER_API_FORCE_INGEST=1`) bypasses gating for parser changes or resolver-coverage improvements that warrant re-evaluation. Adds a nullable `blob_sha` column to `installomator_labels`; existing rows self-heal on the first ingest after migration. **Migration**: `ALTER TABLE installomator_labels ADD COLUMN blob_sha TEXT;` (see deploy notes).
- **Verbatim label body stored on ingest.** A nullable `fragment` column on `installomator_labels` now holds the exact upstream `.sh` body each row was parsed from, alongside the parsed `raw` payload. Lets `generate-label` / `/sources` serve the real label instead of reconstructing it from `raw`, gives the shell resolver the bash to run directly, and preserves provenance. **Migration**: `ALTER TABLE installomator_labels ADD COLUMN fragment TEXT;` (see deploy notes); `--force` a re-ingest to backfill existing rows.
- **OS-portable resolution of dynamic Installomator values**, so ingest can fill `download_url` / `app_new_version` for shell-expression labels without macOS (resolution now runs in place on the Linux host). The `PipelineResolver` learned the high-yield Installomator helpers, all in pure Python: `versionFromGit` (follows the `github.com/.../releases/latest` redirect — no token, no rate limit), `downloadURLFromGit` (releases API, picks the asset by the label's `type`/`archiveName`; honors `PATCHER_API_GITHUB_TOKEN` for the 5000/hr authenticated limit), `xpath` (XPath 1.0 via `lxml`, mimics macOS `/usr/bin/xpath` output so appcast/`sparkle:` pipelines resolve), `getJSONValue` (recursively resolves its nested `$(curl …)` source, then walks the dot/bracket key path with stdlib `json`), and `echo` (expands `${var}` / `$var` references against the label context — resolving the referenced variable's expression recursively — so labels that fetch a feed into one variable and parse it on a later line, like `echo "${updateFeed}" | xpath …`, resolve). The parsed label is threaded through as resolver context so `downloadURLFromGit` can read `type`/`archiveName` and `echo` can resolve sibling variables. Enable per-host with `PATCHER_API_RESOLVE_INGEST=true`; unresolvable values degrade to `NULL` as before. Adds `lxml` as an API dependency. `PATCHER_API_GITHUB_TOKEN` now also authenticates the SHA-gating tree-discovery call (same `api.github.com` 60/hr unauthenticated budget), so discovery no longer 403s once that budget is spent.
- **Resolved values stay fresh across SHA-gated refreshes.** A dynamic value like `appNewVersion=$(versionFromGit …)` drifts upstream even though the label's bash (and SHA) never changes, so SHA gating would otherwise let it go stale. Ingest now re-resolves dynamic `download_url` / `app_new_version` for SHA-unchanged labels from their stored `raw` (no re-fetch, no re-parse) on every run, updating just those two columns. No-op when resolution is disabled; skips labels freshly resolved in the same run and literal-only labels.

### Fixed
- **`GET /apps/{slug}/sources` now actually returns the `jamf_app_installer` source.** The endpoint only wired `installomator`, `homebrew_cask`, and `autopkg` into the `AppSources` response, so `jamf_app_installer` fell back to the schema default of `null` for *every* app regardless of what stitch had matched and stored — the JAI coverage was in the database the whole time but never served. The seed path (`seed.py`) had the same omission and now persists `jamf_app_installer` from `SEED_SOURCES` too.
- **`GET /apps/{slug}/sources` no longer 500s for apps whose AutoPkg recipes lack a `name` or `shortname`.** The `AutopkgRecipeEntry` response schema required both as non-optional `str`, but the upstream index (and the `AutopkgIndexEntry` ingest schema) allow `null` — shared-processor recipes carry `name: null` and some app recipes have no clean `shortname`, which stitch stored faithfully. Pydantic then rejected the stored `None` on the way out, so any such app (e.g. `privileges`) returned a `ValidationError`/500. Both fields are now `str | None`.
- **Jamf App Installers now matches decorated catalog titles to bare app labels.** JAI titles routinely carry a trailing version/year/edition (`Sublime Text 4`, `Microsoft Word 365`, `Adobe Acrobat Reader DC Continuous`) or a leading vendor (`SAP Privileges`, `Cisco Webex`, the JetBrains IDEs) that the Installomator label name omits, so the old exact-normalized-equality match silently missed them — `privileges` never picked up its `SAP Privileges` JAI coverage. The JAI index now keys each row under its exact normalized title *plus* a trailing-decoration-stripped form *plus* a leading-vendor-stripped form (vendor strip gated on a curated allowlist so a genuine two-word name like `Visual Studio` keeps its first word). Exact titles always win over stripped variants. Lifted JAI coverage attachment from 158→241 labels and 182→263 casks against the live catalog. Coverage-indicator only, so a rare edition mismatch is low-cost.
- **Resolved `app_new_version` values are sanity-checked before storage**, closing a gap where `download_url` was validated (against `looks_like_clean_http_url`) but the version field was not. A pipeline that succeeded at the shell level but captured the wrong thing — a whole HTML page, an HTTP header dump, an empty string, or an un-`head`'d multi-line list of every match — was stored verbatim as if it were a version. The new `looks_like_clean_version` validator rejects empty/whitespace-only, multi-line, markup-bearing, over-length (>60 chars), and digit-free values, landing them as `NULL` instead; internal spaces are allowed so legitimate `Build 4200`-style versions survive. Measured against the live catalog: 83 of 492 stored versions were garbage that is now nulled. Values self-heal on the next resolved ingest (no migration).
- **Installomator label parser no longer truncates or collapses shell-expression values** ([#65](https://github.com/liquidz00/Patcher/issues/65)). The non-greedy regex stopped at the first `)` or whitespace it met, corrupting ~790 values across the catalog: pipelines like `$(curl ... | sed -E 's/foo(bar)/x/')` were cut at the inner `)`, parameter expansions like `${rawVersion// build /.}` were cut at the space, and a key assigned twice (resolve-then-transform, arch-conditional branches) kept only the last assignment. The parser is now a quote-aware scanner that tracks shell nesting contexts (single/double quotes, `$(...)` and backtick command subs, `(`/`{` groups), so command substitutions open a fresh quoting context even inside double quotes and values spanning multiple physical lines are read whole. A multi-assigned key is preserved as the ordered list of every assignment in the `raw` JSON; the projected scalar columns (`download_url`, `app_new_version`, …) and `generate-label` take the first assignment (the resolving step / primary value — empirically the arm64 branch in arch-conditional labels). Precise per-architecture resolution remains the resolver's job. **To apply to an existing catalog**, force a re-ingest so SHA-gated labels are re-parsed: `--force` on `scripts/ingest.py` (or `PATCHER_API_FORCE_INGEST=1`).
