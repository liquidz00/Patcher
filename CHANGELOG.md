...
## [v3.3.2] - 2026-06-13
### Added
- **`PatcherClient.analyze(where=...)` pre-filter.** Filter titles by `min_compliance` / `min_hosts` / `released_after` before analyzing, matching the CLI's `analyze` filters.
- **Catalog records now carry the Apple Team ID.** `expected_team_id` is exposed directly on catalog app records (and on matched Installomator labels in your reports), so code-signature verification no longer needs a second `sources` lookup.
- **`NotFoundError` for clean 404 handling.** A 404 from the API client now raises `NotFoundError` (a subclass of `APIResponseError`), so you can catch the not-found case specifically while existing `except APIResponseError` handlers keep working.

### Changed
- **`import patcher` is lighter.** Public symbols are now imported lazily, so library-only consumers no longer pay the pandas / keyring / fpdf import cost up front. `pip install patcherctl` and the public API are unchanged.
- **Patcher no longer keeps a local Installomator label cache.** Label data now comes straight from the catalog. Any leftover `~/Library/Application Support/Patcher/.labels` directory is removed automatically on the next run and by `reset cache`.
- **More consistent app-name matching.** The matcher normalizes punctuated app names the same way the catalog does, so more of your titles resolve to an Installomator label or Homebrew coverage.

### Deprecated
- **`InstallomatorClient` is deprecated** and will be removed in a future release; constructing it now emits a `DeprecationWarning`. Use `PatcherClient` / `PatcherAPIClient` instead (set `PATCHER_API_URL` for self-hosted catalogs).

### Removed
- **The Mac App Store (MAS) catalog source was dropped.** It was dormant and impractical. `mas` no longer appears in any app's sources.
- **The always-null `latest_release_date` field was removed** from catalog app records (it was never populated outside seed data).
- **Removed the unused `DataManager.load_cached_data` and `HTTPClient.set_concurrency`.** Neither were ever called and outbound concurrency is set once at construction via `max_concurrency`.

### Fixed
- **`analyze --excel-file` now actually reads the file.** The path was previously accepted but ignored. You can now re-analyze a previously-exported Excel report directly. Combining it with `--all-time` is rejected with a clear error.
- **Matched Installomator labels in reports now carry their install type, download URL, and Team ID** instead of showing `null` when the catalog had the data.
- **The `installomator` analyze filter no longer miscounts uncovered titles.** Titles with no Installomator label were wrongly listed as covered.
- **A malformed Jamf token response now raises a clear `TokenError`** instead of an unhandled `TypeError`.
- **Resetting credentials in in-memory mode no longer touches the system keychain**, matching the rest of the in-memory credential path.
- **Cache pruning no longer stops at the first undeletable file.** It logs the file and continues pruning the rest.
- **`PatcherAPIClient.get_app_sources` is more robust.** It no longer crashes on shared AutoPkg recipes (those with a null name or shortname), and now exposes the full Jamf App Installer source fields (`bundle_id`, `version`, `jamf_id`, `download_url`, `architecture`) that were previously dropped.
