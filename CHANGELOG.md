...
## [Unreleased]
### Added
- **Jamf App Installers metadata enrichment via the titles API.** Beyond the HTML coverage scrape, `fetch_jai_catalog` pulls Jamf Pro's App Installers *titles* API (OAuth client-credentials; one token covers the whole sweep) — the title list plus bounded-concurrency per-title detail, with each title falling back to its lean record if its detail call fails. The catalog-global title endpoints need no specific tenant. `jamf_app_installers` gains `bundle_id`, `version`, `jamf_id`, `download_url`, `architecture`; `/apps/{slug}/sources` exposes them. Stitch now matches JAI by **`bundle_id` first** (exact, a precision overlay over the name matcher, which stays the workhorse since only ~16% of labels and no casks carry one) and **backfills** JAI's `bundle_id` onto name-matched apps that lack one. **Migration**: `ALTER TABLE jamf_app_installers ADD COLUMN bundle_id TEXT; ADD COLUMN version TEXT; ADD COLUMN jamf_id TEXT; ADD COLUMN download_url TEXT; ADD COLUMN architecture TEXT;` then `CREATE INDEX ix_jamf_app_installers_bundle_id ON jamf_app_installers (bundle_id);`.
