...
class TestListApps:
    @pytest.mark.asyncio
    async def test_list_apps_returns_parsed_records(self):
    ...
    @pytest.mark.asyncio
    async def test_list_apps_forwards_filter_params(self):
    ...
    @pytest.mark.asyncio
    async def test_list_apps_rejects_non_array_payload(self):
        def handler(request: httpx.Request) -> httpx.Response:
...
class TestGetApp:
    @pytest.mark.asyncio
    async def test_get_app_returns_none_on_404(self):
        def handler(request: httpx.Request) -> httpx.Response:
    ...
    @pytest.mark.asyncio
    async def test_get_app_returns_record(self):
        def handler(request: httpx.Request) -> httpx.Response:
            return httpx.Response(200, json=_FIREFOX_RECORD)
    ...
    @pytest.mark.asyncio
    async def test_get_app_sources_returns_parsed_sources(self):
        payload = {
            "installomator": {
...
class TestGenerateLabel:
    @pytest.mark.asyncio
    async def test_generate_label_returns_parsed_response(self):
        payload = {
            "label_name": "firefox",
    ...
    @pytest.mark.asyncio
    async def test_non_404_error_raises_api_response_error(self):
        def handler(request: httpx.Request) -> httpx.Response:
...
class TestListDrift:
    @pytest.mark.asyncio
    async def test_list_drift_returns_parsed_response(self):
        payload = {
            "total_scanned": 5,
            "total_with_drift": 1,
    ...
    @pytest.mark.asyncio
    async def test_list_drift_forwards_filter_params(self):
        seen: dict[str, str] = {}

        def handler(request: httpx.Request) -> httpx.Response:
            seen.update(dict(request.url.params))
            return httpx.Response(
                200, json={"total_scanned": 0, "total_with_drift": 0, "entries": []}
            )
    ...
    @pytest.mark.asyncio
    async def test_list_drift_hits_drift_path_not_slug(self):
...
class TestGetAppDrift:
    @pytest.mark.asyncio
    async def test_get_app_drift_returns_parsed_entry(self):
        def handler(request: httpx.Request) -> httpx.Response:
    ...
    @pytest.mark.asyncio
    async def test_get_app_drift_returns_none_for_null_body(self):
    ...
    @pytest.mark.asyncio
    async def test_get_app_drift_returns_none_on_404(self):
        def handler(request: httpx.Request) -> httpx.Response:
            return httpx.Response(404, json={"detail": "App with slug 'nope' not found"})
    ...
    @pytest.mark.asyncio
    async def test_get_app_drift_raises_on_non_404_error(self):
        def handler(request: httpx.Request) -> httpx.Response:
...
class TestClientBehavior:
    @pytest.mark.asyncio
    async def test_async_context_manager_closes_pool(self):
        def handler(request: httpx.Request) -> httpx.Response:
            return httpx.Response(200, json=[])
    ...
    @pytest.mark.asyncio
    async def test_get_jamf_index_returns_code_to_slug_map(self):
