...
class TestInitialization:
    def test_attach_client_redacts_values_on_validation_error(self, config_manager):
    ...
    def test_token_manager_initialization(self, mock_token, config_manager):
    ...
    def test_save_token(self, mock_set_credential):
...
class TestTokenValid:
    @patch.object(TokenManager, "token", new_callable=PropertyMock)
    def test_token_valid_true(self, mock_token, token_manager):
    ...
    @patch.object(TokenManager, "token", new_callable=PropertyMock)
    def test_token_valid_false(self, mock_token, token_manager):
    ...
    @patch.object(TokenManager, "token", new_callable=PropertyMock)
    def test_token_valid_no_expiration(self, mock_token, token_manager):
...
class TestEnsureValidToken:
    ...
    async def test_ensure_valid_token_valid_token(self, mock_is_expired, token_manager):
    ...
    @pytest.mark.asyncio
    @patch.object(TokenManager, "token", new_callable=PropertyMock)
    async def test_ensure_valid_token_invalid_token_fetch_success(self, mock_token, token_manager):
        mock_token.return_value = AccessToken(
    ...
    @pytest.mark.asyncio
    @patch.object(TokenManager, "token", new_callable=PropertyMock)
    async def test_ensure_valid_token_invalid_token_fetch_failure(self, mock_token, token_manager):
        mock_token.return_value = AccessToken(
            token="expired_token", expires=datetime.now(timezone.utc) - timedelta(seconds=1)
        )
        # Mock token_valid to return False, meaning the token is invalid
