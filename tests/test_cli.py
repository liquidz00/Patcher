...
class TestGroupCallback:
    ...
    @pytest.mark.asyncio
    async def test_cli_runs_setup_when_not_completed(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_cli_noninteractive_bootstrap(self, mocker):
        settings = PatcherSettings(setup_completed=True)
        mocker.patch("src.patcher.cli.PatcherSettings.load", return_value=settings)
        boot = mocker.patch(
            "src.patcher.cli.Setup.bootstrap_noninteractive", new_callable=AsyncMock
        )
    ...
    @pytest.mark.asyncio
    async def test_cli_warns_on_interpreter_mismatch(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_real_patcher_dirs_untouched_after_destructive_command(self):
...
class TestReset:
    @pytest.mark.asyncio
    async def test_reset_invalid_kind_exits_2(self):
    ...
    @pytest.mark.asyncio
    async def test_reset_creds_url_end_to_end(self):
    ...
    @pytest.mark.asyncio
    async def test_reset_full_runs_every_step(self):
        config = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_reset_ui_reconfigures(self):
        setup = MagicMock()
        setup.reset_ui_config.return_value = True
    ...
    @pytest.mark.asyncio
    async def test_reset_single_credential(self, mocker):
        mocker.patch(
    ...
    @pytest.mark.asyncio
    async def test_reset_cache_when_disabled_exits_cleanly(self):
    ...
    @pytest.mark.asyncio
    async def test_reset_full_failure_raises(self, mocker):
        config = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_reset_specific_credential(self, mocker, credential, key):
    ...
    @pytest.mark.asyncio
    async def test_reset_all_credentials(self, mocker):
        mocker.patch("src.patcher.cli.click.prompt", new=AsyncMock(return_value="value"))
        config = MagicMock()
        ctx = _ctx(config=config)
    ...
    @pytest.mark.asyncio
    async def test_reset_cache_clears_when_enabled(self):
        dm = MagicMock()
...
class TestExport:
    @pytest.mark.asyncio
    async def test_export_threads_settings_into_client_and_runs(self, mocker):
...
class TestAnalyze:
    @pytest.mark.asyncio
    async def test_analyze_summary_requires_output_dir(self):
...
class TestDiff:
    @pytest.mark.asyncio
    async def test_diff_builds_client_and_renders(self, mocker):
        mock_client_cls = mocker.patch("src.patcher.cli.PatcherClient")
    ...
    @pytest.mark.asyncio
    async def test_diff_json_output(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_diff_list_snapshots_empty_exits(self, mocker):
        mocker.patch(
            "src.patcher.cli.get_data_manager"
        ).return_value.get_cached_files.return_value = []
        ctx = _ctx()
    ...
    @pytest.mark.asyncio
    async def test_diff_between_two_dates(self, mocker):
        mock_client = mocker.patch("src.patcher.cli.PatcherClient").return_value
...
class TestDrift:
    @pytest.mark.asyncio
    async def test_drift_builds_client_and_renders(self, mocker):
        mock_client_cls = mocker.patch("src.patcher.cli.PatcherClient")
    ...
    @pytest.mark.asyncio
    async def test_drift_none_reports_no_drift(self, mocker):
        mock_client = mocker.patch("src.patcher.cli.PatcherClient").return_value
    ...
    @pytest.mark.asyncio
    async def test_drift_json_output(self, mocker):
        mock_client = mocker.patch("src.patcher.cli.PatcherClient").return_value
        result = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_drift_json_null_when_no_result(self, mocker):
        mock_client = mocker.patch("src.patcher.cli.PatcherClient").return_value
        mock_client.detect_drift = AsyncMock(return_value=None)
        ctx = _ctx()
    ...
    @pytest.mark.asyncio
    async def test_drift_single_entry_renders_detail(self, mocker):
