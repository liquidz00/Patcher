...
class TestPersistence:
    ...
    def test_save_uses_camelcase_alias(self, tmp_path):
        path = tmp_path / "patcher.plist"
...
class TestMigration:
    def test_migrate_v2_enable_installomator(self):
        s = PatcherSettings.model_validate(
            {"enable_installomator": False, "interpreter_path": "/x"}
        )
        assert s.enable_matching is False
        assert s.integrations.installomator is False

    def test_migrate_v2_enable_homebrew(self):
        s = PatcherSettings.model_validate({"enable_homebrew": True, "interpreter_path": "/x"})
        assert s.integrations.homebrew is True

    def test_migrate_v1_nested_format(self, tmp_path):
        path = tmp_path / "patcher.plist"
        v1 = {
    ...
    def test_migrate_v1_missing_ui_keys_fall_back_to_defaults(self, tmp_path):
        path = tmp_path / "patcher.plist"
        with path.open("wb") as f:
